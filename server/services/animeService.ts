...
export const searchAnime = async (query: string): Promise<AnimeType[]> => {
    ...
    if (cachedSearch) {
        const {data: cachedAnimes, error:cachedAnimesError } = await supabase
        ...
        if (cachedAnimes && cachedAnimes.length > 0) {
            ...
            const anilistIds = cachedAnimes.map((row: any) => row.anilist_id)
    ...
    try {
        ...
        const result: AnimeDetailType = {
            ...
            characters: anime.characters.edges.map((edge: any) => ({
                ...
                .filter((actor:any) => actor.language === 'JAPANESE' || actor.language === 'ENGLISH')
                .map((actor: any) => ({
                    id: actor.id,
        ...
        const {data: insertedCharacters, error: insertedCharactersError} = await supabase
            .from('characters')
        ...
        const {error: upsertVoiceActorsError} = await supabase
        .from('voice_actors')
